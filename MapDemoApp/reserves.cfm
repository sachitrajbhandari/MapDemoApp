     ...
     <div id="map" class=" panel-body">
      ...
      <div class="card">
        <div class="card-header">
          <h4 class="">Reserves of Tasmania</h4>
        </div>
        <div class="card-body">
          <cfoutput>  
            <cfquery name="myQuery" datasource="awspostgres"> 
                SELECT * FROM public.list_local_govt_reserves_statewide ORDER BY feat_name DESC 
            </cfquery>
            <cfgrid format="html"
                name="maingrid" 
                query="myQuery" 
                pagesize="25" 
                autowidth="true" 
                href="reserves_sel.cfm?id="
                hrefKey="cid" 
                sort="yes"
                includeBootstrap="no"
                >
                <cfgridcolumn name="cid" display="no" />
                <cfgridcolumn name="category" header="Category" />
                <cfgridcolumn name="feat_name" header="Feature Name" />
                <cfgridcolumn name="shape_area" header="Shape Area" />
            </cfgrid>
          </cfoutput>
        </div>
      </div>
      ...
      <div class="card shadow mb-4">
        <div class="card-header py-3">
          <h4 >Total Reserves Area by Category</h4>
        </div>
         <cfoutput>  
        <cfquery name="myGraphQuery" datasource="awspostgres">
          SELECT category as category_list, ROUND(sum(shape_area), 0) as shape_area_list FROM public.list_local_govt_reserves_statewide group BY category
        </cfquery> 
      </cfoutput>
        <div class="card-body">
          <div class="chart-bar">
...
var my_shape_area_list1 = new Array(6); 
var my_category_list1 = new Array(6); 

<cfset myLoop = 0> 
<cfoutput> 
  <cfloop query="myGraphQuery"> 
    my_category_list1 [#variables.myLoop #] = "#myGraphQuery.category_list#"; 
  ...
  options: {
    ...
    scales: {
      ...
      yAxes: [{
        ticks: {
          ...
          // Include a dollar sign in the ticks
          callback: function(value, index, values) {
            return number_format(value) + ' sq. m';
          }
        },
        gridLines: {
          color: "rgb(234, 236, 244)",
    ...
    tooltips: {
      ...
      callbacks: {
        label: function(tooltipItem, chart) {
          var datasetLabel = chart.datasets[tooltipItem.datasetIndex].label || '';
          return datasetLabel + ': ' + number_format(tooltipItem.yLabel) + ' sq.m';
        }
      }
    },
  }
